...
import { Coordinate, Coordinates } from '../shared/new/grid';
...
class TilePaintInfo {  
  ...
  constructor(InitialColor: 0|1 = 0){    
...
export class PaintRobot{
  ...
  constructor(program: number[], initialPanelColor: 0|1 = 0){
    this.currentDirection = [0,1];
    this.currentPosition = [0,0];
    this.workInfo = {};
    this.intComputer = new IntCodeComputer(program);
    this.intComputer.addInput(initialPanelColor);
    this.currentTile = new TilePaintInfo(initialPanelColor);
  }
  ...
  get currentTile(): TilePaintInfo {
    return this.workInfo[this.posKey];
  }
  
  set currentTile(tile: TilePaintInfo){
    this.workInfo[this.posKey] = tile;
  }

  get currentColor(): 0|1{
    return this.currentTile.Color;
  }
  ...
  get currentCoordinates(): Coordinates{
    ...
    Object.keys(this.workInfo).map(key =>{
      const c = new Coordinate();
      c.importFromCoordKey(key,this.workInfo[key].Color);
      coordinateList.push(c);
    });  
  ...
  paint(paintInput: 0 | 1): void {    
    this.currentTile.Color = paintInput;
    this.currentTile.NrOfTimesPainted++;
  }
  ...
  turnAndMove(moveOutput: 0 | 1): void{
    ...
    if(!this.currentTile) this.currentTile = new TilePaintInfo(0);
  }
}


export const solveInput = (programInput: Input): any => {
